...
class Main(QDialog):
    ...
    def process_data(self):
        if validation_of_data():
            min_elem = find_element()
            summ = find_sum_of_first_row()
            if summ > min_elem[1]:
                list_of_numbers[min_elem[0]] = str(summ)
            ...
            # -*- выполнение задания -*-
                ...
                for i, elem in enumerate(list_of_numbers):
                    self.plainTextEdit.insertPlainText(str(elem) + " ")
    ...
    def save_data_in_file(self):
        ...
        if path_to_file:
            ...
            for i, e in enumerate(list_of_numbers):
                file.write(e + " ")
                if ((i + 1) % 5 == 0):
                    file.write("\n")
...
def find_element():
    min = [0,float(list_of_numbers[0])]
    for index, item in enumerate(list_of_numbers):
        if min[1] > float(item):
            print(min[1], float(item))
            min[0] = index
            min[1] = float(item)
    return min
...
def find_sum_of_first_row():
    ...
    while i < 5:  # в строке должно быть ровно 6 чисел
